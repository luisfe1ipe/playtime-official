...
<div wire:key="{{$notification->id}}" class="flex items-center w-full gap-4 p-2 rounded-lg cursor-pointer hover:bg-zinc-950/80">
  <div class="flex w-full gap-1">
    <img class="rounded-full size-10" src="{{$notification->data['find_player_creator_user']['photo']}}"
      alt="Foto {{$notification->data['find_player_creator_user']['name']}}">
    <div class="w-full">
      <div class="text-sm flex-nowrap">
        <a href="#" class="text-primary-400 hover:underline">
          {{"@".$notification->data['find_player_creator_user']['nick']}}
        </a>
        <p>
