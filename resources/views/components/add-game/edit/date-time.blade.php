...
<div class="flex items-center border rounded-lg border-zinc-700">
    ...
    <div class="relative w-full text-center">
        <input @if (array_key_exists($text, $array) && array_key_exists('start', $array[$text])) value="{{ $array[$text]['start'] }}" @endif
            wire:model='days_times_play.{{ $text }}.start' type="time"
    ...
    <div class="relative w-full text-center">
        <input @if (array_key_exists($text, $array) && array_key_exists('end', $array[$text])) value="{{ $array[$text]['end'] }}" @endif type="time"
            wire:model='days_times_play.{{ $text }}.end'
